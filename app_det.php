...
// MATT WAS HERE, DISMISS CASES IF BUTTON HAS BEEN PRESSED
if ($dis)
	{
	$q = "UPDATE lcm_app SET dismissed=".$dis." WHERE id_app =".$app."  LIMIT 1 ";
	lcm_query($q);
	}
...
//	echo _Ti('app_input_type') . _Tkw('appointments', $row['type']) . "<br />\n";
	//MATT WAS HERE, ONLY SHOW DATE, NOT TIME
	echo "Date: " . format_date($row['start_time'], 'date_short'). "<br />\n";
...
//			. "</a><br />\n";
	...
	if ($ac['e'])
		...
		if ($row['dismissed']==0)
			{
			echo '<a href="app_det.php?dis=1&app=' . $row['id_app'] . '" class="run_lnk">' . _T('app_button_dis') . "</a>\n";
			}
		}
	if ($row['id_case'] > 0) {
...
//		}
//MATT WAS HERE. FOLLOWUPS NOT CREATEABLE FROM APPOINTMNETS
//		// Show child followup
...
//		echo "</a><br />\n";
		...
		echo '<a href="case_det.php?case=' . $row['id_case'] . '&amp;tab=general" class="edit_lnk">' . 'Return to the case' . "</a>\n"; // TRAD
	}
