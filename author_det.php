...
if ($author_data['status']=='group')
	{
	lcm_page_start(_T('title_group_view') . ' ' . $fullname, '', '', 'authors_intro');
	matt_page_start(_T('title_group_view'));
	}
else
	{
	lcm_page_start(_T('title_author_view') . ' ' . $fullname, '', '', 'authors_intro');
	matt_page_start(_T('title_author_view'));
	}
...
//		echo '<fieldset class="info_box">';
		...
		switch ($tab) {
			...
			case 'general' :
				...
				if ($author_data['status']!='befriender'&&$author_data['status']!='befriender')
					{
					echo '<li>Access? ' . ($author_data['right1']?"<strong>Yes</strong>":"No") . "</li>\n";
					echo '<li>Edit? ' . ($author_data['right2']?"<strong>Yes</strong>":"No") . "</li>\n";
				...
				if (($GLOBALS['author_session']['right3']) ||
					($author == $GLOBALS['author_session']['id_author']))
						echo '<a href="edit_author.php?author=' . $author . '" class="edit_lnk">'
							."Edit this Infomation</a>\n";
				...
				if ($author_data['status']=='befriender')
					...
					$result = lcm_query('
							select 
								cl.*,
								stfu.date_start as start,
								enfu.date_start as end
							from lcm_case as c
							left join lcm_case_client_org as cco on cco.id_case = c.id_case
							left join lcm_client as cl on cl.id_client = cco.id_client
							left join (select * from lcm_followup where type="followups35")as stfu on stfu.id_case = c.id_case
							left join (select * from lcm_followup where type="followups22")as enfu on enfu.id_case = c.id_case
							where 
								c.type_case = "befriender" and 
								c.notes ='.$author_data['id_author'].'
								');
					...
					for ($i = 0 ; (($i<$prefs['page_rows']) && ($row = lcm_fetch_array($result))) ; $i++) {
						...
						echo "<td class='tbl_cont_" . ($i % 2 ? "dark" : "light") . "'>";
						echo ($row['start']?format_date($row['start'],'date_short'):'Broken?');
						echo "</td>\n";

						echo "<td class='tbl_cont_" . ($i % 2 ? "dark" : "light") . "'>";
						echo ($row['end']?format_date($row['end'],'date_short'):'Ongoing');
						echo "</td>\n";
