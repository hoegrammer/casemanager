...
if (empty($_SESSION['errors'])) {
	...
	if ($_GET['app']>0) {
		...
		$q = "SELECT cl.name_first, cl.name_last FROM lcm_client as cl LEFT JOIN lcm_case_client_org as cco ON cl.id_client = cco.id_client WHERE cco.id_case = "._session('id_case'); 	
		$result=lcm_query($q);
		while ($row = lcm_fetch_array($result)) 
			{
			$matt = $row['name_first'] . ' '. $row['name_last'];
			}
		$_SESSION['form_data']['title'] = $matt;
...
//		$_SESSION['form_data']['reminder']   = $time;
		$_SESSION['form_data']['dismissed#'] = '0';
...
if (_session('id_app', 0) > 0)
	...
	matt_page_start(_T('title_app_edit'));
	}
else
	{
...
<?php
	...
	echo "<td>" . f_err_star('start_time') . "Date: " . "</td>\n";
	echo "<td>";

	$name = ($edit ? 'start' : '');
	echo get_date_inputs($name, _session('start_time'), false);
// MATT WAS HERE, ONLY GET DATE, NOT TIME
...
?>
		...
		<!-- End time 
		// MATT WAS HERE. ALL THIS STUFF IS HIDDEN, (APPOINTMENTS DO NOT HAVE DURATION)
		<tr>
//	<?php
//
...
//		}
// MATT WAS HERE, REMOVING "ADD NEW CLIENTS TO THIS APPOINTMENT STUFF (BELOW) AND DELETE CLIENTS FROM THIS APPOINTMENT STUFF (ABOVE)
//		echo "\t\t\t$q\n";
