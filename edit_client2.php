...
if ($id_client > 0) {
	lcm_page_start();
	matt_page_start('Edit Client...');
} else {
	lcm_page_start();
	matt_page_start('Add New Client...');
}


if ($GLOBALS['author_session']['right2']=='0')
	{
	no_tabs();
	echo "<p>Sorry, you are not allowed to contribute.</p>";
	matt_page_end();
	lcm_page_end();
	exit;
	}
...
echo '<form name="editcli" action="upd_client.php" method="post" onSubmit="var but = document.forms[\'editcli\'][\'submit\']; but.disabled=true; but.innerHTML=\'Please wait\'; return true;">';
...
$fu->printEdit();
if ($id_client ==0 || $mode=='scores')
	{
	echo '<input type="hidden" name="mode" id="mode" value="scores"/>';
	}
else
	{
	echo '<input type="hidden" name="mode" id="mode" value="edit"/>';
	}
$obj_client = new LcmClientInfoUI(0,0,$id_client);
$obj_client->printEdit($mode);
...
echo "<button id=\"mr_submit\" name=\"submit\" type=\"submit\" value=\"submit\" class=\"simple_form_btn\">" . _T('button_validate') . "</button>\n";
echo '<input type="hidden" name="ref_edit_client" value="<?php echo $_SERVER[\'HTTP_REFERER\'] ?>" />';
echo '</form>';
