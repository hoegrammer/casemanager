...
function auth() {
	...
	if (isset($_COOKIE['lcm_session'])) {
		if (verifier_session($_COOKIE['lcm_session'])) {
			if ($author_session['status'] != 'pending' AND $author_session['status'] != 'trash' AND $author_session['status'] != 'suspended') {
	...
	$query = "SELECT * FROM lcm_author WHERE username='$auth_login' AND status !='external' AND status !='host'";
