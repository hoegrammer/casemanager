...
class LcmCase extends LcmObject {
	...
	function validate() {
		...
		if (! $this->getDataString('type_case'))
			$errors['type_case'] = "NO CASE TYPE!";
	...
	function save() {
		...
		$fl = "
			title='"              . $this->getDataString('title')           . "',
			...
			type_case='"         . $this->getDataString('type_case')        . "'";
...
//MATT WAS HERE. GETTING RID OF "STAGE" DB STUFF
...
//MATT WAS HERE. REMOVE "ASSIGNMENT" FOLLOW-UP, REPLACED BY MANDATORY FIRST FOLLOW-UP.
...
//BUG ABOVE? 
			// Add lcm_stage entry
// MATT WAS HERE. REMOVE CODE TO RECORD CASE STAGES, AS SEEMED USELESS
...
//			lcm_query($q);
		...
		//MATT WHAS HERE. UPDATE CLIENT "LAST WORK" WHEN CASE DETAILS EDITED.
...
// MATT WAS HERE. REMOVED "TIME SPENT ON CASE" FROM CASE INFO PAGE.
...
//				//MATT WAS HERE, REMOVING DUD OPTIONS FROM CASE STATE CHANGE LIST (MERGED, ETC)
//				if (($futype ==	"opening")||(($this->getDataString('status')!='draft')&&($futype =="conclusion"))||($futype =="draft"))
//				{	
...
//			echo "</form>\n";
//MATT WAS HERE, UP TO SOMETHING. REMOVING "CHANGE CASE STAGE" FROM CASE DETAILS PAGE 
			// Show case stage
//			echo "<form action='edit_fu.php' method='get'>\n";
//			echo "<input type='hidden' name='case' value='" . $this->getDataInt('id_case') . "' />\n";
...
//		}
		...
		if (($this->data['status'] == 'closed')||($this->data['status']=='draft')) {
			...
			if ($row_tmp) {
				echo '<li>';
				echo '<div style="background: #f0f0f0; padding: 4px; border: 1px solid #aaa;"><p>';
				// MATT WAS HERE. SHOW OUTCOME AND AMOUNT IN "CONCLUSION" BOX ON SHOW CASE DETAILS SCREEN.
				if ($row_tmp['outcome'])
					{
					$zots = get_kw_from_id($row_tmp['outcome']);
					echo 'Outcome: '.$zots['title'];
					echo '</p><p>';
					}
				if ($row_tmp['outcome_amount']!=0)
					{
					echo 'Outcome Amount £: '.$row_tmp['outcome_amount'];
					echo '</p><p>';
					}
				echo 'Conclusion: ';
...
//MATT WAS HERE. HIDE COLABRATION INFO FROM CASE VIEWING PAGE.
...
//		echo "</ul>\n";
	...
	function printBasic($show_subtitle = true, $allow_edit = true) {
		// Read site configuration preferences
		$case_assignment_date = read_meta('case_assignment_date');
		$case_alledged_crime  = read_meta('case_alledged_crime');
		$case_legal_reason    = read_meta('case_legal_reason');
...
//			lcm_bubble('new_case');
		...
		echo '<input type="hidden" name="title" id="input_case_title" value="n/a">';		
...
//		echo "</td></tr>\n";
		...
		if ($special=='accomidation')
			{
			echo '<input type="hidden" id="type_case" name="type_case" value="Accomidation"/>';
			}	
		elseif ($special=='support')
			{
			echo '<input type="hidden" id="type_case" name="type_case" value="Support"/>';
			}	
		elseif ($special=='befriender')
			{
			echo '<input type="hidden" id="type_case" name="type_case" value="Befriender"/>';
			}	
		elseif ($special=='section4')
			{
			echo '<input type="hidden" id="type_case" name="type_case" value="Section 4"/>';
			}	
		else
			{
			echo '<input type="hidden" id="type_case" name="type_case" value="Something Else"/>';
			}

//		if ($special==1)
//			{
//			echo '<input name="new_kwg_case_id[]" value="27" type="hidden">';
//			echo '<input type="hidden" id="new_keyword_case0" name="new_keyword_case_value[]" value="82"/>';
//			echo '<input type="hidden" id="input_stage" name="stage" value="investigation"/>';
//			}
//		else
//			{
			// Legal reason
	...
	//		echo '<td><textarea name="case_notes" id="input_case_notes" class="frm_tarea" rows="3" cols="100">'
	...
	//		//MATT WAS HERE CURRENTLY		
	//		// Case status MATT WAS HERE, REMOVING CASE STATUS (OPEN/DRAFT) FROM EDITING/CREATION PAGE.
	//		if (!($this->getDataInt('id_case'))) 
	//			{
	...
	//			$statuses = ($this->getDataInt('id_case') ? array('draft','open','suspended','closed','merged') : array('open','draft') );
	...
	//		else
	//			{
				echo '<input type="hidden" name="status" id="input_status" value="open"/>';
	//			}
			
			// Case stage MATT WAS HERE, REMOVEING CASE STAGE FROM CASE EDITING/CREATING PAGES.
			if (!$special)
				{
				if (! $this->getDataString('stage'))
				...
				{	
				echo '<input type="hidden" name="stage" id="input_stage" value="submitted"/>';
				}

			
			// Public access rights
			// FIXME FIXME FIXME  // MATT WAS HERE, REMOVE I/O RIGHTS BOXES TO SAVE CLUTTER.
			echo '<input type="hidden" name="pub_write" id="case_public_write" value="yes" />';
			echo '<input type="hidden" name="public" id="public_read" value="yes" />';
...
//MATT WAS HERE. SPECIAL VERSION OF "PRINTFOLLOWUPS" FUNCTION TO DISPLAY SIMPLE FORMATTING FOR PRINTING.
	function mattPrintFollowups($show_filters = false) {
		for ($cpt = 0,$this->getFollowupStart(); (! $this->getFollowupDone()); $cpt++) {
			$item = $this->getFollowupIterator();
			show_matt_listfu_item($item, $cpt);
		}
...
class LcmCaseListUI extends LcmObject {
	...
	function start($listtype=0,$admin=false) {
		show_listcase_start($listtype,$admin);
	}

	function printList($listtype=0,$admin=false) { //listype = 0 normal, 1 = accom subm'ed, 2 = accom waitlist, 3 = ?

		global $prefs;
		// Select cases of which the current user is author
		$q = "SELECT 
				foo.bar, cco.id_client, ap.title as ttl, max(fu.date_start) as date_start, fu.description,
				c.legal_reason, c.amount, c.id_case, cl.name_first, cl.name_last, cl.check1, cl.check2, cl.check3, cl.pannel, 
				cl.accom, kw.title, if(foo.bar='1','z',c.stage) as stage, c.public, c.pub_write, c.date_creation, c.date_update, c.type_case, c.status
			FROM 
				lcm_case as c 
			NATURAL JOIN 
				lcm_case_author as a 
			NATURAL LEFT JOIN 
				lcm_keyword_case as kc 
			LEFT JOIN 
				lcm_keyword as kw ON kc.id_keyword = kw.id_keyword
			LEFT JOIN 
			...
			LEFT JOIN
				(
				select a.* from lcm_app as a where a.dismissed = false
				) as ap on ap.id_case = c.id_case";

		if($this->search !='')
			{
			$x = preg_replace('/ +/', ' ', $this->search);
			$q2 = " AND ((name_first LIKE '%$x%')
					OR (name_middle LIKE '%$x%')
					OR (name_last LIKE '%$x%')
					OR (CONCAT(name_first, ' ', name_middle, ' ', name_last) LIKE '%$x%')
					OR (CONCAT(name_first, ' ', name_last) LIKE '%$x%')
					OR (CONCAT(name_last, ' ', name_first) LIKE '%$x%')
					OR (citizen_number LIKE '%$x%')
					OR (date_birth LIKE '%$x%')
					OR (nass_number LIKE '%$x%')
		...
		if ($listtype==21)
			{
			$q .= " LEFT JOIN (SELECT id_case, if(max(to_days(date_start))is null,0,if(max(to_days(date_start))=to_days(now()),1,'')) as bar from lcm_followup where type='followups40' group by id_case)as foo on foo.id_case = c.id_case ";
			}
		else
			{
			$q .= " LEFT JOIN (SELECT 0 as id_case, 1 as bar) as foo on c.id_case = foo.id_case";
			}
		if ($listtype==1)
			{
			$q .= " INNER JOIN lcm_followup as fu on fu.id_case = c.id_case ";
			$q .= " WHERE ((fu.type='followups17')or(fu.type='followups28'))";
			$q .= $q2;
			$q .= " AND c.type_case = \"Accomidation\" ";
			$q .= " AND c.stage = \"Submitted\" ";
			$q .= " AND c.status = \"Open\" ";
			}
		elseif (($listtype==2) || ($listtype==12))
			{
			$q .= " LEFT JOIN lcm_followup as fu on fu.id_case = c.id_case ";
//			$q .= " WHERE ((fu.type='followups18')or(fu.type='followups28'))";
			$q .= " WHERE ((fu.type='followups18'))";//or(fu.type='followups28'))";
			$q .= $q2;
			$q .= " AND c.type_case = \"Accomidation\" ";
			$q .= " AND c.stage = \"Waiting List\" ";
			$q .= " AND c.status = \"Open\" ";
			}
		elseif ($listtype==3)
			{
			$q .= " LEFT JOIN lcm_followup as fu on fu.id_case = c.id_case ";
			$q .= " WHERE fu.type='followups24'";
			$q .= $q2;
			$q .= " AND c.type_case = \"Accomidation\" ";
			$q .= " AND c.stage = \"accom\" ";
			$q .= " AND c.status = \"Open\" ";
		...
		elseif ($listtype==5)
			{
			$q .= " LEFT JOIN lcm_followup as fu on fu.id_case = c.id_case ";
			$q .= " WHERE fu.type='followups22'";
			$q .= " AND c.type_case = \"Accomidation\" ";
			}
		elseif ($listtype==6)
			{
			$q .= " LEFT JOIN lcm_followup as fu on fu.id_case = c.id_case ";
			$q .= " WHERE fu.type='followups19'";
			$q .= $q2;
			$q .= " AND c.type_case = \"Accomidation\" ";
			$q .= " AND c.stage = \"Rejected\" ";
		...
		elseif ($listtype==21)
			...
			$q .= " LEFT JOIN lcm_followup as fu on fu.id_case = c.id_case ";
			$q .= " WHERE ((fu.type='followups17')or(fu.type='followups28'))";
			$q .= " AND c.type_case = \"Support\" ";
			$q .= " AND (c.stage = \"Submitted\" OR c.stage = \"Submitted2\" OR c.stage = \"Submitted3\" OR c.stage = \"Submitted4\") ";
			$q .= " AND c.status = \"Open\" ";
//			$q .= " AND fu.date_start >= date_sub(curdate(), interval 1 month)";
			}
		elseif ($listtype==22)
			{
			$q .= " LEFT JOIN lcm_followup as fu on fu.id_case = c.id_case ";
			...
			$q .= $q2;
			$q .= " AND c.type_case = \"Support\" ";
			$q .= " AND (c.stage = \"Submitted\" OR c.stage = \"Submitted supported\") ";
			$q .= " AND c.status = \"Open\" ";
			$q .= " AND fu.date_start < date_sub(curdate(), interval 1 month)";
			}
		elseif ($listtype==23)
			{
			$q .= " LEFT JOIN lcm_followup as fu on fu.id_case = c.id_case ";
			$q .= " WHERE fu.type='followups20' or fu.type='followups24'";
			$q .= " AND ((c.type_case = \"Support\" AND c.stage = \"Supported\" AND c.status = \"Open\")";
			$q .= " OR ( c.type_case=\"Accomidation\" AND c.stage= \"Accom\" ))";
			}
		elseif ($listtype==24)
			{
			$q .= " LEFT JOIN lcm_followup as fu on fu.id_case = c.id_case ";
...
//			$q .= " AND " . $q_owner; 
//
//			//MATT WAS HERE. ADD SQL WHERE CLAUSE FOR SHOWING ONLY OPEN/CLOSED CASES IN CASELISTING
...
//				if ($this->date_start)
//					$q .= " AND c.date_update >= '" . $this->date_start . "'";
//
//				if ($this->date_end)
//					$q .= " AND c.date_update <= '" . $this->date_end . "'";
//			} else {
//				if ($prefs['case_period'] < 1900) // since X days
//					$q .= " AND " . lcm_query_subst_time('c.date_update', 'NOW()') . ' < ' . $prefs['case_period'] * 3600 * 24;
//				else // for year X
//					{
//	//				$q .= " AND " . lcm_query_trunc_field('c.date_creation', 'year') . ' = ' . $prefs['case_period'];
//					$q .= " AND c.date_update >= '".$prefs['case_period']."-04-01'";
//					$q .= " AND c.date_update < '".($prefs['case_period']+1)."-04-01'";
//					}
//					//MATT WAS HERE, CHOOSE CASES TO LIST BASED ON HOW RECENTLY THEY WERE WORKED ON, NOT CREATED.
//			}
			}
		$q .= ' group by cco.id_case';
	//		if ($prefs['case_type']=="Accomidation")
	//			{
	//			$q .= " AND c.type_Case = \"Accomidation\"";
	//			}	
		//
...
//MATT WAS HERE ATTN: ORDERBY CLAUSE GETS GENERATED HERE...
		// Sort cases by creation date

		if ($sort_allow[_request('fudate')])
			$sort_clauses[] = "fu.date_start " . _request('fudate');
		elseif ($sort_allow[_request('accom_order')])
			$sort_clauses[] = "cl.accom " . _request('accom_order');
		elseif ($sort_allow[_request('pannel_order')])
			$sort_clauses[] = "cl.pannel " . _request('pannel_order');
		elseif ($sort_allow[_request('amount_order')])
			$sort_clauses[] = "c.amount " . _request('amount_order');
		elseif ($sort_allow[_request('status_order')])
			$sort_clauses[] = "c.stage " . _request('status_order');
		elseif ($sort_allow[_request('by_order')])
			$sort_clauses[] = "c.type_case " . _request('by_order');

		//		if ($sort_allow[_request('status_order')])
//			$sort_clauses[] = "status " . _request('status_order');
//		elseif ($sort_allow[_request('case_order')])
//			$sort_clauses[] = 'c.date_creation ' . _request('case_order');
//		elseif ($sort_allow[_request('update_order')])
//			{
//			$sort_clauses[] = "c.date_update " . _request('update_order');
//			}
//		elseif ($sort_allow[_request('case_details')])
//			{
//			$sort_clauses[] = "kw.title " . _request('case_details');
//			}
//		else
//			$sort_clauses[] = 'c.date_update DESC'; // default
		...
		for ($i = 0; (($i<$prefs['page_rows']) && ($row = lcm_fetch_array($result))); $i++)
			show_listcase_item($row, $i, $this->search,'__DEFAULT__','',$listtype,$admin);
