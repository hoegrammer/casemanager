...
function show_all_keywords_type($type = 'system') {
	...
	foreach ($kwg_all as $kwg)
		...
		if ($kwg['type']=='system')
			{
			if (($kwg['name']=='_crimresults') ||
				($kwg['name']=='_exptypes') ||
				($kwg['name']=='appointments') ||
//				($kwg['name']=='conclusion') ||
				($kwg['name']=='sentence')
//				($kwg['name']=='stage')
				)
	...
	if ($type == 'user')
		echo '<p><a href="keywords.php?action=edit_group&amp;id_group=0" class="create_new_lnk click_btn">'
			. _T('keywords_button_kwg_new') . '</a></p>' . "\n";
	elseif ($type == 'contact')
		echo '<p><a href="keywords.php?action=edit_group&amp;type=contact&amp;id_group=0" class="create_new_lnk click_btn">'
			. _T('keywords_button_kwg_new') . '</a></p>' . "\n";
}
...
function show_keyword_group_id($id_group, $id_parent = 0) {
	...
	if (! $id_group) {
		...
		if ($id_parent) {
			...
			matt_page_start(_T('title_subkwg_edit'));
			no_tabs();
		} else {
			...
			matt_page_start(_T('title_kwg_new'));
			no_tabs();
		}
		...
		matt_page_start(_T('title_kwg_edit'));
		no_tabs();
...
matt_page_start(_T('menu_admin_keywords'), '', '', 'keywords_intro');
