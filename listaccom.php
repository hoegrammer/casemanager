...
lcm_page_start('Accommodation Team Management');
matt_page_start('Accommodation Team Managmenet');
$admin = false;
...
//		'terminated' => 'Previously Supported',
//		'all' => 'All'
		);
...
switch ($tab)
	...
	case 'outstanding':
		include('notes.php');
		show_notes('post-accom','listaccom.php',$admin);
		break;
	case 'new':
		$type='caselist';
		$text='List of clients who have submitted an application to the Accommodation Team in the last month.';
		$listtype=1;
		break;
	case 'wait':
		$type='caselist';
		$text='All clients on the accommodation Waiting List.';
		$listtype=2;
		break;
	case 'rejected':
		$type='caselist';
		$text='All clients whose accommodation application was rejected.';
		$listtype=6;
		break;
	case 'supported':
		$type='caselist';
		$text='List of clients currently being accomidated.';
		$listtype=3;
		break;
	case 'all':
		$type='caselist';
		$text='All Accommodation Team cases, including current, old and rejected cases.';
		$listtype=4;
		break;
	case 'reviewdue':
		$type='applist';
		$text='List upcomming reviews.';
		$listtype='accrev';
		break;
	case 'terminated':
		$type='caselist';
		$text='List of clients no longer accomidated.';
		$listtype='5';
		break;
	case 'termdue':
		$type='applist';
		$text='List of clients due for termination.';
		$listtype='accterm';
		break;
	}
...
if ($type=='caselist')
	{
	$find_accom_string = trim($_GET['find_accom_string']);
	show_find_box('accom', $find_accom_string,'','',$tab );
	echo '<p class="normal_text">' . "\n";
	$case_list = new LcmCaseListUI();
	$case_list->setSearchTerm($find_accom_string);
...
if ($type=='applist')
	...
	$q .=
		"
		AND fu.type = 'followups24'
		AND dismissed = false
		AND ap.title='".$listtype."'
		ORDER BY fu.date_start";
...
//			echo '<td class="tbl_cont_' . ($i % 2 ? 'dark' : 'light') . '">'
//				. ($listtype=='accrev'?	
//					'<a href="edit_fu.php?case='.$row['id_case'].'&ctype=accomidation&type=review&ref=listaccom.php?tab=reviewdue" class="edit_lnk">Review Now</a>' :
//					'<a href="edit_fu.php?case='.$row['id_case'].'&ctype=accomidation&type=stage_change&stage=terminated&ref=listaccom.php?tab=termdue" class="edit_lnk">Terminate Now</a>'
////					)
	//			. '</td>';
//			echo '<td class="tbl_cont_' . ($i % 2 ? 'dark' : 'light') . '">'
