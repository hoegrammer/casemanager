...
$types_period = array('m1' => 30, 'm3' => 91, 'm6' => 182, 'y1' => 365, 'y5' => '1825'); // 30 days, 3 months, 6 months, 1 year
...
// MATT WAS HERE! ADDED ADDED A _REQUEST TO GRAB STATE OF 'CASE STATUS' FILTER FIELD FROM GET
if (($v = _request('case_status')))
	{
	$prefs['case_status']= $v;
	$prefs_change = true;
	}
...
$q_dates = "SELECT DISTINCT if(month(date_creation)>3, YEAR(date_creation),YEAR(date_creation)-1) as year FROM lcm_case ORDER BY year";
...
while($row = lcm_fetch_array($result)) 
	...
	echo '<option value="' . $row['year'] . '"' . $sel . '>' . 'Year '.$row['year'].'-'. ($row['year']+1) . "</option>\n";
	}

echo "</select>\n";
echo ' which are ';
// MATT WAS HERE, SET A DEFUAULT "CASE STATUS" FOR NEW USERS.
if (!($prefs['case_status']))
	{
	$prefs['case_status'] == 'open';
	}

//MATT WAS HERE. ADD A "CASE STATUS" DROPDOWN LISTBOX TO THE CASE FILTERS.
echo '<select name="case_status">';
echo '<option value="all" ' . isSelected($prefs['case_status'] == 'all') . '>All</option>';
echo '<option value="open" ' . isSelected($prefs['case_status'] == 'open') . '>Open</option>';
echo '<option value="closed" ' .isSelected($prefs['case_status'] == 'closed')  .'>Closed</option>';
echo '<option value="draft" ' .isSelected($prefs['case_status'] == 'draft') .'>One-off</option>';
echo '</select>';
